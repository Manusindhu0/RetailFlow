...
export function CheckoutPanel({ onSuccess }: CheckoutPanelProps) {
  ...
  const [discountOpen, setDiscountOpen] = useState<{ id: number; type: "percent" | "flat" } | null>(null);
  ...
  return (
    <div className="w-full flex-1 flex flex-col min-h-0 bg-card">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto flex flex-col gap-4 p-5 min-h-0">
        ...
        <button 
          ...
          className="w-full flex items-center gap-2 p-2.5 bg-secondary/30 hover:bg-secondary/60 rounded-xl border border-border/50 transition-colors text-left"
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            {cart.customerPhone && (
              <span className="text-xs text-muted-foreground block truncate">{cart.customerPhone}</span>
            )}
          </div>
        ...
        {cart.activePaymentMode === "cash" && (
          <div>
            <label className="text-xs font-semibold text-muted-foreground mb-1.5 block uppercase tracking-wide">
              Cash Received (₹)
              <span className="text-muted-foreground/50 ml-2 font-normal normal-case">(Press Enter to Charge)</span>
            </label>
            <input
              type="number"
              min={total}
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && cart.items.length > 0) handleCharge();
              }}
              className="w-full px-3 py-2.5 bg-secondary border border-border rounded-xl text-sm
                         focus:outline-none focus:border-primary/60 transition-colors"
      ...
      <div className="p-5 border-t border-border space-y-2 flex-shrink-0 bg-card">
        {mutation.isError && (
        ...
        >
          ...
          {cart.items.length > 0 && !mutation.isPending && (
             <kbd className="hidden ml-2 px-2 py-0.5 rounded bg-black/20 text-white/90 text-xs font-mono sm:inline-block">F9</kbd>
          )}
        </button>
